...
class NovelAnalysisWorker(QThread):
    ...
    def _analysis_prompt(self):
        return (
            ...
            "应把新片段里的变化写入已有人物 notes、设定 description、伏笔状态/章节或 project_materials；"
            "不要把已有档案里没有在本片段推进的信息当作新发现重复输出。\n"
            "如果这是长文分块的一部分，只提取本片段明确出现或能稳定判断的信息；"
            "即使只是片段，也尽量为 project_materials 写出本片段可确定的资料草案。\n"
            "JSON 结构：{\n"
            ...
            "world_rules 写世界观、势力、制度、能力/道具规则；timeline 按时间顺序记录关键事件；"
            "summary 写本片段对全局剧情的简明摘要。没有明确内容就留空字符串。"
        )
    ...
    def _editorial_quality_prompt(self):
        ...
        if self.action in summary_actions:
            return common + (
                ...
                "本章摘要控制在 1-3 句；关键事实控制在 3-6 条短句，只保留会影响后文续写的变化。"
            )
        return common + (
    ...
    def _action_prompt(self):
        prompts = {
            ...
            "summary": (
                ...
                "本章摘要写 1-3 句，约 120-180 字以内，只概括核心事件和情绪转折。"
                "关键事实写 3-6 条短句，每条只写一个可继承事实，删掉背景解释、重复信息和创作建议。"
