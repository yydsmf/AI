...
GitHub Actions 会在打包时把 `1.0.13` 写入程序版本和 Windows 安装器版本。
...
Windows 在线更新采用“先下载、再确认安装”的可见流程：用户在程序内点击“下载新版安装包”后，主程序保持打开并显示下载进度；安装包下载到系统“下载”目录后，再提示用户是否立即退出并安装。用户确认后，程序会退出并自动打开新版安装包。
...
安装器会随包携带 Python 运行所需的 VC++ DLL，并在安装阶段补齐 Microsoft Visual C++ 运行库。在线更新下载阶段在主程序内可见执行，避免主程序提前退出后用户无法判断是否正在下载。
...
当前方案是免费、可控的安装包更新方式：程序检查新版本，Windows 在主程序内先下载安装包并显示进度，下载完成后由用户确认退出并打开安装向导；macOS 下载 DMG 后打开安装包。
